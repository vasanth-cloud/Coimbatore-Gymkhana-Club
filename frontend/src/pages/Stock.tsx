...
export const Stock: React.FC = () => {
  ...
  const [lockToggling, setLockToggling] = useState<boolean>(false);
  ...
  const loadLockStatus = async (dateStr: string) => {
    try {
      const res = await stockApi.getDailyLockStatus(dateStr);
      setIsDateLocked(res.is_locked);
    } catch (err) {
      console.error('Failed to load daily lock status:', err);
  ...
  const handleToggleDailyLock = async () => {
    try {
      ...
      const res = await stockApi.toggleDailyLock({
        lock_date: ledgerDate,
        is_locked: !isDateLocked,
      });
      setIsDateLocked(res.is_locked);
      alert(
        res.is_locked
          ? `🔒 Date ${ledgerDate} is now Finalized & Locked! Edits are disabled.`
          : `🔓 Date ${ledgerDate} is now Unlocked for editing.`
      );
    } catch (err: any) {
      alert(err.response?.data?.detail || 'Failed to update lock status');
    } finally {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {/* Lock Status Badge */}
                  {isDateLocked ? (
                    <span className="px-2.5 py-1 bg-rose-500/10 text-rose-400 border border-rose-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-md shrink-0 ${
                      isDateLocked
                    ...
                    title={isDateLocked ? "Unlock this date to allow edits" : "Mark this date as finalized/done to protect from edits"}
                  >
                    {lockToggling ? (
                    ...
                    <span>{isDateLocked ? 'Unlock to Edit' : 'Mark as Finalized & Lock Date'}</span>
                  </button>
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (isDateLocked) {
                      alert(`Date ${ledgerDate} is locked. Please click 'Unlock to Edit' to record daily stock.`);
                      return;
                    }
                    handleOpenBulkDailyModal();
                  }}
                  className={`px-3.5 py-1.5 font-black rounded-xl text-xs flex items-center gap-1.5 shadow-lg transition-all shrink-0 ${
                    isDateLocked
                  ...
                  title={isDateLocked ? "This date is locked" : "Bulk record/backdate daily stock figures for all drinks on selected date"}
                >
                          ...
                          <td className="py-2.5 px-2 text-center border-l border-[#30363d]">
                            {isDateLocked ? (
                              <div className="flex items-center justify-center">
                                <span className="px-2 py-0.5 text-[10px] font-bold text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-lg flex items-center gap-1 cursor-not-allowed" title="Date is locked. Click 'Unlock to Edit' at the top to make changes.">
